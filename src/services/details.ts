...
import { RANGE_INTERVAL_MAP } from '@/constants';
  ...
  return useQuery<AssetHistory[], Error>({
    ...
    queryFn: async () => {
      const data = await axios.request({
        ...
        params: {
          interval: RANGE_INTERVAL_MAP[range].interval,
          ...
          start: dayjs().subtract(RANGE_INTERVAL_MAP[range].dateRange, 'hours').valueOf(),
